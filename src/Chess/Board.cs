...
namespace Chess
{
    ...
    public class Board
    {
        ...
        public Board()
        {
            ...
            this.charBoard = initialCharBoard;
            arrayToBitboards();
        }
        ...
        public void arrayToBitboards()
        {
            for(int i = 0; i < 64; i++)
            {
                ...
                switch(this.charBoard[i / 8, i % 8])
                {
                    case 'r': this.BR.bb += convertStringToBitboard(bin);
                        break;
                    case 'n': this.BN.bb += convertStringToBitboard(bin);
                        break;
                    case 'b': this.BB.bb += convertStringToBitboard(bin);
                        break;
                    case 'q': this.BQ.bb += convertStringToBitboard(bin);
                        break;
                    case 'k': this.BK.bb += convertStringToBitboard(bin);
                        break;
                    case 'p': this.BP.bb += convertStringToBitboard(bin);
                        break;
                    case 'R': this.WR.bb += convertStringToBitboard(bin);
                        break;
                    case 'N': this.WN.bb += convertStringToBitboard(bin);
                        break;
                    case 'B': this.WB.bb += convertStringToBitboard(bin);
                        break;
                    case 'Q': this.WQ.bb += convertStringToBitboard(bin);
                        break;
                    case 'K': this.WK.bb += convertStringToBitboard(bin);
                        break;
                    case 'P': this.WP.bb += convertStringToBitboard(bin);
                        break;
                }
            }
            this.drawBoardArray();
        ...
        public void drawBoardArray()
        {
            for(int i = 0; i < 64; i++)
            {
                if (((WK.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "K"; }
                if (((WQ.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "Q"; } 
                if (((WR.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "R"; } 
                if (((WN.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "N"; } 
                if (((WB.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "B"; } 
                if (((WP.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "P"; } 
                if (((BK.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "k"; } 
                if (((BQ.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "q"; } 
                if (((BR.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "r"; } 
                if (((BN.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "n"; } 
                if (((BB.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "b"; } 
                if (((BP.bb>>i)&1)==1) { this.charBoard[i / 8, i % 8] = "p"; } 

                if (String.IsNullOrEmpty(this.charBoard[i / 8, i % 8])){this.hessBoard[i / 8, i % 8] = " ";}
            }
            this.printCharBoard();
        }
        ...
        {
            for (int i = 0; i < this.charBoard.GetLength(0); i++)
            {
                for (int j = 0; j < this.charBoard.GetLength(1); j++)
                {
                    Console.Write(this.charBoard[i,j] + ",");
                }
                Console.WriteLine();
            }
        }
